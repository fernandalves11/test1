...
export class UpholdBot {
  ...
  public start(): void {
    ...
    this.intervalId = setInterval(async () => {
      for (const pair of this.botConfig.getPairs()) {
        ...
        const currentPrice = await this.priceServ.getPriceExternal(pair);
        ...
        const hasAlert = this.alertServ.checkOscillationForAlert(change, this.botConfig.getThreshold());
        if (hasAlert) {
          const alertConfig = this.alertServ.triggerAlert(pair, change, previous, currentPrice, this.botConfig.getBotConfigId());
  ...
  async init() {
    ...
    if(botConfigId < 0) Logger.error("Bot config id is invalid! Current id: ", botConfigId);
  }
}